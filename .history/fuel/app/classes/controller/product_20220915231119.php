...
class Controller_Product extends Controller_Template
{
	...
	public function action_show_list() {
		...
		$products = DB::query('SELECT'id', 'name', 'price', 'company_id')->from('products')->execute()->as_array();
		$view->set('subtitle', 'サンプル株式会社の在庫にある商品一覧です。');
		$view->set('contents', date('Y年m月d日') . '現在時点');
		$view->set('products', DB::select('id', 'name', 'price', 'company_id')->from('products')->execute()->as_array());
