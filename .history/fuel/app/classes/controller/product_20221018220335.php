...
class Controller_Product extends Controller_Layout
{
	public function action_test() {
		$data['product'] = DB::select('products.id', 'name', 'price', 'company_id', 'companies.id', 'company_name')
								->from('products')
								->where('id', '=', '1')
								->execute();
		$this->template->content = VIew::forge('product/test', $data);
	}
	// 新規登録：画面表示
	public function action_form() {
		$data['companies'] = Model_Company::getList();
		debug::dump($data['companies']);
