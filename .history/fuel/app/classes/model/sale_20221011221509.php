...
class Model_Sale extends \Orm\Model
{
	...
	public static function getData() {
		...
		$datas = DB::select(DB::expr("date_format('created_at', '%Y-%m') as month, count(created_at) as count, sum(discount) as discount, sum(sale) as sale"))
								->from(sales)
								->group_by(DB::expr("date_format(month, '%Y-%m')"))
								->execute()
								->as_array();
		// $datas = DB::select(DB::expr("date_format('created_at', '%Y-%m') as 'month', count('created_at') as count"),
		// 										DB::expr("date_format('created_at', '%Y-%m') as 'month', sum('discount') as discount"), 
		// 										DB::expr("date_format('created_at', '%Y-%m') as 'month', sum('sale') as sale"))
		// 						->from('sales')
		// 						->execute()
		// 						->as_array();
		return $datas;
