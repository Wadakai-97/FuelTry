...
class Model_Sale extends \Orm\Model
{
	 ...
	 * 1営業日前
	 */
	public static function BusinessDay($timestamp) {
		$notWorkDays = [
			'20221019', '20221020', '20221021', '20221022',
		];
		$exTimestamp = date('Ymd', strtotime('-1 day'));
    if(in_array(date('Ymd', $exTimestamp), $notWorkDays, true)) {
        return $exTimestamp;
    }
    return BusinessDay($timestamp);

		// $i = '1';
		// $ex = date('Ymd', strtotime('-' . $i . ' day'));
		// while(in_array($ex, $holiday)) {
		// 	$i++;
		// 	$ex = date('Ymd', strtotime('-' . $i . ' day'));
		// }
		// return $ex;		
