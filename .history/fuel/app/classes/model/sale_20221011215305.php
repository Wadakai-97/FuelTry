...
class Model_Sale extends \Orm\Model
{
	...
	public static function getData() {
		...
		// switch($type) {
		// 	case year:
				$datas = DB::select(DB::expr("date_format('created_at', '%Y') as 'year', count('created_at') as count)",DB::expr("date_format('created_at', '%Y') as 'year', count('created_at') as count), sum('discount') as total_discount, sum('sale') as total_sale"))
										->from('sales')
										->execute()
										->as_array();
			// case monthly:
