...
class Model_Sale extends \Orm\Model
{
	 ...
	 * 1営業日前
	 */
	public static function two_working_days_ago($today, $holidays) {
	...
	public static function two_working_days_ago($one, $holidays) {
		$two = date('Ymd', strtotime('-1day', strtotime($one)));
		for($i='1'; in_array($two, $holidays); $i++) {
			$two = date('Ymd', strtotime('-' . $i . ' day', strtotime($one)));
		}
		return $two;		
	}
}
