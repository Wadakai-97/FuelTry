...
class Model_Sale extends \Orm\Model
{
	...
	public static function monthly() {
		$sales = DB::select(DB::expr("date_format('created_at', '%Y-%m') as 'month', count('created_at') as count, sum('discount') as total_discount, count('created_at') as count,"), )
								->from('sales')
								->execute()
								->as_array();
	}
	...
	public static function total() {
		try {
			...
			$sales = DB::select(DB::expr('SUM(id) AS t_order'), DB::expr('SUM(discount) AS t_dis'), DB::expr('SUM(sale) AS t_sales'))
									->from('sales')
									->execute()
									->as_array();
