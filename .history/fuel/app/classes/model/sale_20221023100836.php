...
class Model_Sale extends \Orm\Model
{
	 ...
	 * 1営業日前
	 */
	public static function BusinessDay($today, $holidays) {
		$one = date('Ymd', strtotime('-1 day'));
		for($i='1'; in_array($one, $holidays); $i++) {
			$one = date('Ymd', strtotime('-' . $i . ' day'));
		}
		return $one;
	}
	public static function two($today, $holidays) {
		$two = date('Ymd', strtotime('-1 day'));
